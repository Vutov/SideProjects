...
{
 internal Dictionary < string , Tickеt > Dict = new Dictionary < string , Ticket > ( );
...
{
 // Look video for brother of my wedding: https://www.youtube.com/watch?v=OXgcs_MeAHI AirTicket ticket = new AirTicket(parwaaz_number, from, to, airline, dt, pp); string result = AddDeleteTicket(ticket, true); if (result.Contains("added"))
{
 airTicketsCount++;
}
 ...
 if (t.FromToKey == fromToKey)
{
 ...
 string ticketsAsString = ReadTickets(ticketsFound); return ticketsAsString;
}
else
{
 return "Not found";
...
string allParameters = line.Substring(firstSpaceIndex + 1);
 string[] parameters = allParameters.Split( new char[] {';
...
parameters = allParameters.Split( new char[] {';
'}, StringSplitOptions.RemoveEmptyEntries);
 for (int i = 0; i < parameters.Length; i++) 
{
 parameters[i] = parameters[i].Trim();
 ...
 for (int i = 0; i < parameters.Length; i++) 
{
 ...
 cd2 = AddTrainTicket(parameters[0], parameters[1], parameters[2], parameters[3], parameters[4]);
 break;
 case "DeleteTrain":allParameters = line.Substring(firstSpaceIndex + 1);
parameters = allParameters.Split( new char[] {';
'}, StringSplitOptions.RemoveEmptyEntries);
for (int i = 0; i < parameters.Length; i++)
{
...
cd2 = DeleteTrainTicket(parameters[0], parameters[1], parameters[2]);
break;
 ...
 for (int i = 0; i < parameters.Length; i++) 
{
 ...
 cd2 = basKaTicketKaIzafah(parameters[0], parameters[1], parameters[2], parameters[3], parameters[4]);
 break;
 case "DeleteBus":
allParameters = line.Substring(firstSpaceIndex + 1);
parameters = allParameters.Split( new char[] {';
 ...
 for (int i = 0; i < parameters.Length; i++) 
{
 ...
 cd2 = DeleteBusTicket(parameters[0], parameters [ 1 ] , parameters[2], parameters[3]);
 break;
 case "FindTickets":
allParameters = line.Substring(firstSpaceIndex + 1);
 parameters = allParameters.Split( new char[] {';
...
cd2 = findTicketsInInterval(parameters[0],parameters[1]);
break;
}
...
{
 return AddAirTicket(parwaaz_number, from, to, airline, dateTime.ToString("dd.MM.yyyy HH:mm"), price.ToString());
}
...
{
 return AddTrainTicket(from,to,dateTime.ToString("dd.MM.yyyy HH:mm"),price.ToString(),studentPrice.ToString());
}
...
{
 return basKaTicketKaIzafah(from, to, Sayahat_ki_Tanzeem, dateTime.ToString("dd.MM.yyyy HH:mm"), price.ToString());
}
